...
export default function AdminDashboard() {
  ...
  const rentalRequestsData = getRentalRequests();
  ...
  const handleAddCar = (carData) => {
    const newCar = {
      ...
      location: carData.location || 'Lagos', // Default location
      imageUrl: carData.images?.[0] || 'https://images.unsplash.com/photo-1552519507-da3b142c6e3d?w=400'
    };
    addRentalCar(newCar);
  ...
  const handleApproveRentalRequest = (requestId) => {
    const result = approveRentalRequest(requestId);
    if (result.success) {
  ...
  const handleDenyRentalRequest = (requestId) => {
    ...
    const result = denyRentalRequest(requestId, reason || 'Request denied by admin.');
    if (result.success) {
      alert(result.message);
    }
  };
  ...
  return (
    <div className="bg-gradient-to-b from-green-50 to-green-100 min-h-screen">
      ...
      <div className="sticky top-16 lg:top-20 z-40 bg-gradient-to-b from-green-50 to-green-100 px-4 mx-auto sm:px-6 lg:px-8 pt-8 pb-4">
        <div className="flex justify-center space-x-1 bg-white p-1 rounded-lg shadow-sm max-w-xl mx-auto">
          <button
          ...
          <main className="flex-1">
            {activeTab === 'rental-requests' ? (
              !selectedRentalRequest ? (
                <div className="bg-white rounded-lg shadow-sm">
                  <div className="p-6 border-b border-gray-200">
                    <h1 className="text-2xl font-bold text-gray-900">Car Listing Requests</h1>
                    <p className="text-gray-600 mt-1">Review and manage car listing submissions from owners</p>
                  </div>
                        ...
                        {filteredRentalRequests.map(request => (
                          <tr key={request.id} className="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              ...
                              <div className="text-sm text-gray-500">{request.car.licensePlate}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                              {request.owner.city}, {request.owner.state}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                              ₦{request.pricing.dailyRate?.toLocaleString()}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                              ...
                              {request.status === 'approved' && !request.addedToFleet && (
                                <button
                                  onClick={() => handleAddToFleet(request.id)}
                              ...
                              {request.status === 'approved' && request.addedToFleet && (
                                <span className="text-xs text-gray-500 italic">Added to Fleet</span>
                              )}
                            </td>
                          </tr>
                        ))}
                ...
                <div className="bg-white rounded-lg shadow-sm">
                  <div className="p-6 border-b border-gray-200">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <h1 className="text-2xl font-bold text-gray-900">Car Listing Request Details</h1>
                  ...
                  <div className="p-6 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-3">
                            <div>
                              ...
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.owner.name}</p>
                            </div>
                            <div>
                              <label className="text-sm font-medium text-gray-500">Email Address</label>
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.owner.email}</p>
                            </div>
                            <div>
                              <label className="text-sm font-medium text-gray-500">Phone Number</label>
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.owner.phone}</p>
                            </div>
                          </div>
                          <div className="space-y-3">
                            <div>
                              <label className="text-sm font-medium text-gray-500">Address</label>
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.owner.address}</p>
                            </div>
                            <div>
                              <label className="text-sm font-medium text-gray-500">City</label>
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.owner.city}</p>
                            </div>
                            <div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          <div>
                            ...
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.make} {selectedRentalRequest.car.model}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">Year</label>
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.year}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">License Plate</label>
                            <p className="text-sm text-gray-900 mt-1 font-mono bg-white px-2 py-1 rounded border">{selectedRentalRequest.car.licensePlate}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">Color</label>
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.color}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">Mileage</label>
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.mileage?.toLocaleString()} km</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">Transmission</label>
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.transmission}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">Fuel Type</label>
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.fuelType}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">Seats</label>
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.seats} passengers</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">Category</label>
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.category}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-4">
                          ...
                          <p className="text-sm text-gray-900 mt-1 bg-white p-3 rounded border">{selectedRentalRequest.car.description}</p>
                        </div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          <div>
                            ...
                            <p className="text-lg font-bold text-gray-900 mt-1">₦{selectedRentalRequest.pricing.dailyRate?.toLocaleString()}</p>
                          </div>
                          {selectedRentalRequest.pricing.weeklyRate && (
                            <div>
                              <label className="text-sm font-medium text-gray-500">Weekly Rate</label>
                              <p className="text-lg font-bold text-gray-900 mt-1">₦{selectedRentalRequest.pricing.weeklyRate?.toLocaleString()}</p>
                            </div>
                          )}
                          {selectedRentalRequest.pricing.monthlyRate && (
                            <div>
                              <label className="text-sm font-medium text-gray-500">Monthly Rate</label>
                              <p className="text-lg font-bold text-gray-900 mt-1">₦{selectedRentalRequest.pricing.monthlyRate?.toLocaleString()}</p>
                            </div>
                          )}
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                          <div>
                            ...
                            <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.pricing.availability}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-500">Delivery Service</label>
                            <div className="flex items-center mt-1">
                              <div className={`w-3 h-3 rounded-full mr-2 ${selectedRentalRequest.pricing.deliveryOption ? 'bg-green-400' : 'bg-red-400'}`}></div>
                              <span className="text-sm text-gray-900">
                                {selectedRentalRequest.pricing.deliveryOption ? 'Available' : 'Not Available'}
                              </span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-6">
                        {selectedRentalRequest.images && selectedRentalRequest.images.length > 0 ? (
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {selectedRentalRequest.images.map((image, index) => (
                              <div key={index} className="aspect-w-4 aspect-h-3">
                                <img
                                  src={image}
                                  alt={`Vehicle photo ${index + 1}`}
                                  className="w-full h-32 object-cover rounded-lg border shadow-sm hover:shadow-md transition-shadow duration-200"
                                />
                              </div>
                            ))}
                          </div>
                    ...
                    {selectedRentalRequest.status === 'pending' && (
                      <div className="border-t border-gray-200 pt-6">
                        <div className="flex flex-col sm:flex-row gap-4">
                          <button
                            onClick={() => {handleApprove(selectedRentalRequest.id); setselectedRentalRequest(null);}}
                          ...
                          <button
                            onClick={() => {handleDeny(selectedRentalRequest.id); setselectedRentalRequest(null);}}
                    ...
                    {/* Denial Reason */}
                    {selectedRentalRequest.status === 'denied' && selectedRentalRequest.denyReason && (
                      <div className="border-t border-gray-200 pt-6">
                        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                          <h3 className="text-lg font-medium text-red-800 mb-2">Denial Reason</h3>
                          <p className="text-red-700">{selectedRentalRequest.denyReason}</p>
                        </div>
                      </div>
                    )}

                    {/* Add to Fleet Button for Approved Requests */}
                    {selectedRentalRequest.status === 'approved' && !selectedRentalRequest.addedToFleet && (
                      <div className="border-t border-gray-200 pt-6">
                        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                          <h3 className="text-lg font-medium text-green-800 mb-2">Request Approved</h3>
                          <p className="text-green-700 mb-4">This car listing has been approved. You can now add it to your rental fleet to make it available for customers to rent.</p>
                          <button
                            onClick={() => handleAddToFleet(selectedRentalRequest.id)}
                            className="px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition-colors duration-200 shadow-sm"
                          >
                            Add to Rental Fleet
                          </button>
                        </div>
                    ...
                    {/* Already Added to Fleet Status */}
                    {selectedRentalRequest.status === 'approved' && selectedRentalRequest.addedToFleet && (
                      <div className="border-t border-gray-200 pt-6">
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                          <h3 className="text-lg font-medium text-blue-800 mb-2">Added to Fleet</h3>
                          <p className="text-blue-700">This car has been successfully added to your rental fleet and is now available for customers to rent.</p>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              // Rental Requests Management Tab
              !selectedRentalRequest ? (
                <div className="bg-white rounded-lg shadow-sm">
                  <div className="p-6 border-b border-gray-200">
                    <h1 className="text-2xl font-bold text-gray-900">Rental Requests</h1>
                    <p className="text-gray-600 mt-1">Manage rental booking requests from customers</p>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center">
                                ...
                                <div>
                                  ...
                                  <div className="text-sm text-gray-500">{request.car.type}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    ...
                    {filteredRentalRequests.length === 0 && (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-600">
                          {rentalRequestFilter === 'all' ? 'No rental requests have been submitted yet.' : `No ${rentalRequestFilter} rental requests found.`}
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="bg-white rounded-lg shadow-sm">
                  <div className="p-6 border-b border-gray-200">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-gray-600 mt-1">Request ID: #{selectedRentalRequest.id}</p>
                      </div>
                      <span className={`inline-flex px-3 py-1 text-sm font-semibold rounded-full ${getStatusColor(selectedRentalRequest.status)}`}>
                        {selectedRentalRequest.status.charAt(0).toUpperCase() + selectedRentalRequest.status.slice(1)}
                      </span>
                  ...
                  <div className="p-6 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-3">
                            <div>
                              ...
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.customerName}</p>
                            </div>
                            <div>
                              <label className="text-sm font-medium text-gray-500">Email Address</label>
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.customerEmail}</p>
                            </div>
                            <div>
                              <label className="text-sm font-medium text-gray-500">Phone Number</label>
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.customerPhone}</p>
                            </div>
                          </div>
                          <div className="space-y-3">
                            <div>
                              <label className="text-sm font-medium text-gray-500">Request Date</label>
                              <p className="text-sm text-gray-900 mt-1">{new Date(selectedRentalRequest.requestDate).toLocaleDateString()}</p>
                            </div>
                            <div>
                              <label className="text-sm font-medium text-gray-500">Pickup Location</label>
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.pickupLocation || 'Not specified'}</p>
                            </div>
                            <div>
                              <label className="text-sm font-medium text-gray-500">Special Requirements</label>
                              <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.specialRequirements || 'None'}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div>
                      <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                        <svg className="w-5 h-5 mr-2 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m0 0V5a2 2 0 012 2v6m0 0v8a2 2 0 01-2 2H6a2 2 0 01-2-2v-8m0 0V7a2 2 0 012-2h4a2 2 0 012 2v0" />
                        </svg>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                ...
                                <p className="text-sm text-gray-900 mt-1">
                                  {selectedRentalRequest.car.year} {selectedRentalRequest.car.make} {selectedRentalRequest.car.model}
                                </p>
                              </div>
                              <div>
                                <label className="text-sm font-medium text-gray-500">Category</label>
                                <p className="text-sm text-gray-900 mt-1">{selectedRentalRequest.car.type}</p>
                              </div>
                              <div>
                                <label className="text-sm font-medium text-gray-500">Rental Start Date</label>
                                <p className="text-sm text-gray-900 mt-1">{new Date(selectedRentalRequest.startDate).toLocaleDateString()}</p>
                              </div>
                              <div>
                                <label className="text-sm font-medium text-gray-500">Rental End Date</label>
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-900 mt-1">
                                  {Math.ceil((new Date(selectedRentalRequest.endDate) - new Date(selectedRentalRequest.startDate)) / (1000 * 60 * 60 * 24))} days
                                </p>
                              </div>
                              <div>
                                <label className="text-sm font-medium text-gray-500">Total Cost</label>
                                <p className="text-sm font-bold text-green-600 mt-1">₦{selectedRentalRequest.totalCost?.toLocaleString()}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    {selectedRentalRequest.status === 'pending' && (
                      <div className="border-t border-gray-200 pt-6">
                        <div className="flex flex-col sm:flex-row gap-4">
                          <button
                            onClick={() => {
                              handleApproveRentalRequest(selectedRentalRequest.id);
                          ...
                          <button
                            onClick={() => {
                              handleDenyRentalRequest(selectedRentalRequest.id);
                              setSelectedRentalRequest(null);
                            }}
                            className="flex-1 px-6 py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 transition-colors duration-200 shadow-sm"
                          >
                    ...
                    {selectedRentalRequest.status === 'approved' && (
                      <div className="border-t border-gray-200 pt-6">
                        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                          <h3 className="text-lg font-medium text-green-800 mb-2">Request Approved</h3>
                          <p className="text-green-700">This rental request has been approved. The customer has been notified and can proceed with the rental.</p>
                        </div>
                      </div>
                    )}
                  </div>
